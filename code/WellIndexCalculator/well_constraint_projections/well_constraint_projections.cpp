...
void WellConstraintProjections::real_roots(Eigen::VectorXd coeffs, Eigen::VectorXd *real_roots)
{
...
QList<QVector3D *> WellConstraintProjections::move_points_4p(QList<QVector3D *> coords, double d, QVector3D *s)
{
    ...
    QVector3D *x0moved = new QVector3D(*WellIndexCalculator::GeometryFunctions::project_point_to_plane(coords.at(0),s,&top_plane_point));
    QVector3D *x1moved = new QVector3D(*WellIndexCalculator::GeometryFunctions::project_point_to_plane(coords.at(1),s,&top_plane_point));
    QVector3D *x2moved = new QVector3D(*WellIndexCalculator::GeometryFunctions::project_point_to_plane(coords.at(2),s,&bot_plane_point));
    QVector3D *x3moved = new QVector3D(*WellIndexCalculator::GeometryFunctions::project_point_to_plane(coords.at(3),s,&bot_plane_point));
...
QList<QVector3D *> WellConstraintProjections::move_points_3p(QList<QVector3D *> coords, double d, QVector3D *s)
{
    ...
    QVector3D *x0moved = new QVector3D(*WellIndexCalculator::GeometryFunctions::project_point_to_plane(coords.at(0),s,&top_plane_point));
    QVector3D *x1moved = new QVector3D(*WellIndexCalculator::GeometryFunctions::project_point_to_plane(coords.at(1),s,&bot_plane_point));
    QVector3D *x2moved = new QVector3D(*WellIndexCalculator::GeometryFunctions::project_point_to_plane(coords.at(2),s,&bot_plane_point));
...
{
    QVector3D *x0 = new QVector3D(*WellIndexCalculator::GeometryFunctions::XYZ_to_qvec(coords.at(0)));
    QVector3D *x1 = new QVector3D(*WellIndexCalculator::GeometryFunctions::XYZ_to_qvec(coords.at(1)));
    QVector3D *x2 = new QVector3D(*WellIndexCalculator::GeometryFunctions::XYZ_to_qvec(coords.at(2)));
    QVector3D *x3 = new QVector3D(*WellIndexCalculator::GeometryFunctions::XYZ_to_qvec(coords.at(3)));
    QVector3D *normal_vector = s;
    normal_vector->normalize();
    ...
    QVector3D *x0moved = new QVector3D(*WellIndexCalculator::GeometryFunctions::project_point_to_plane(x0,s,&top_plane_point));
    QVector3D *x1moved = new QVector3D(*WellIndexCalculator::GeometryFunctions::project_point_to_plane(x1,s,&top_plane_point));
    QVector3D *x2moved = new QVector3D(*WellIndexCalculator::GeometryFunctions::project_point_to_plane(x2,s,&bot_plane_point));
    QVector3D *x3moved = new QVector3D(*WellIndexCalculator::GeometryFunctions::project_point_to_plane(x3,s,&bot_plane_point));
...
QList<QVector3D*> WellConstraintProjections::kkt_eq_solutions(arma::mat A, arma::vec b)
{
    ...
    for(int i=0; i<3; i++){
        ...
        for(int jj = 0; jj < eigenvalue_solutions.length(); jj++){
            QVector3D *temp_solution = new QVector3D(eigenvalue_solutions.at(jj)(0),
...
double WellConstraintProjections::shortest_distance(QList<QVector3D *> coords)
{
    ...
    QVector3D* P0 = new QVector3D(*coords.at(0));
    QVector3D* P1 = new QVector3D(*coords.at(1));
    QVector3D* Q0 = new QVector3D(*coords.at(2));
    QVector3D* Q1 = new QVector3D(*coords.at(3));
...
QList<Eigen::Vector3d> WellConstraintProjections::non_inv_solution(Eigen::Matrix3d A, Eigen::Vector3d b)
{
    ...
    //std::cout << "here is vector s0 " << std::endl << s0 << std::endl << "with length = " << sqrt(s0(0)*s0(0)+s0(1)*s0(1)+s0(2)*s0(2))  << std::endl;
    //std::cout << "here is vector s1 " << std::endl << s1 << std::endl << "with length = " << sqrt(s1(0)*s1(0)+s1(1)*s1(1)+s1(2)*s1(2))  << std::endl;

    /*
    // TEST PRINTING HERE

    // we have that pAq = lu, or eqiv A = p^t l u q^t
    Eigen::Matrix3d p = lu.permutationP();
    Eigen::Matrix3d q = lu.permutationQ();

    // l and u stored in same matrix. sepparate them.
    // l has ones on diagonal
    Eigen::Matrix3d l = Eigen::Matrix3d::Identity();
    l.triangularView<Eigen::StrictlyUpper>() = lu.matrixLU();
    Eigen::Matrix3d u;
    u.triangularView<Eigen::Lower>() = lu.matrixLU();
    std::cout << "Here is the matrix A:"<< std::endl << A << std::endl;

    std::cout << "These column vectors span null space:"<< std::endl << null_space << std::endl;

    std::cout << "Here is, up to permutations, its LU decomposition matrix:"
         << std::endl << lu.matrixLU() << std::endl;
    std::cout << "Here is the P part:" << std::endl;
    std::cout << p << std::endl;
    std::cout << "Here is the Q part:" << std::endl;
    std::cout << q << std::endl;
    std::cout << "Here is the L part:" << std::endl;
    std::cout << l << std::endl;
    std::cout << "Here is the U part:" << std::endl;
    std::cout << u << std::endl;
    u = WellConstraintProjections::rm_entries_eps(u,10e-10);
    std::cout << "Here is the U part with small entries removed:" << std::endl;
    std::cout << u << std::endl;
    std::cout << "Let us now reconstruct the original matrix A:" << std::endl;
    std::cout << lu.permutationP().inverse() * l * u * lu.permutationQ().inverse() << std::endl;
    */
    // END TEST PRINTING HERE
    ...
    for (int ii=0;ii<6;ii++){
        if(comproots[ii]==0){
            ...
            if(sqrt(s(0)*s(0) + s(1)*s(1) +s(2)*s(2)) < 1+tolerance && sqrt(s(0)*s(0) + s(1)*s(1) +s(2)*s(2))>1-tolerance){
                solution_vectors.append(new QVector3D(s(0),s(1),s(2)));
...
QList<QVector3D*> WellConstraintProjections::interwell_projection_4p(QList<QVector3D*> coords, double d)
{
    ...
    for (int ii=0;ii<6;ii++){
        if(comproots[ii]==0){
            ...
            invmatr(2,2) = 1.00/(eigval(2)-cur_root);
            arma::vec s = eigvec*invmatr*eigvec_trans*b;
            current_vector = new QVector3D(s(0),s(1),s(2));
            double tolerance = 0.001;
    ...
    for(int eig_num=0; eig_num<3; eig_num++){
        ...
        for (int i=0; i<s_list.length(); i++){
            QVector3D *current_vector = new QVector3D(s_list.at(i)(0),s_list.at(i)(1),s_list.at(i)(2));
...
QList<QVector3D *> WellConstraintProjections::well_distance_projection(QVector3D *heel, QVector3D *toe, double max, double min, double epsilon)
{
    ...
    // Distance too long
        ...
        moved_heel = *heel + move_distance*heel_to_toe_vec;
        moved_toe =  *toe  - move_distance*heel_to_toe_vec;
        projected_points.append(new QVector3D(moved_heel));
        projected_points.append(new QVector3D(moved_toe));
    }
