...
protected:
    ...
    virtual void SetUp() {
        ...
        x1 = new Model::Reservoir::Grid::XYZCoordinate(-4,20,0);
        x2 = new Model::Reservoir::Grid::XYZCoordinate(2,2,0);
        x3 = new Model::Reservoir::Grid::XYZCoordinate(-1,-2,-3);
        x4 = new Model::Reservoir::Grid::XYZCoordinate(1,-20,2);
        x5 = new Model::Reservoir::Grid::XYZCoordinate(0,0,0.2);
        coords.append(x1);
        coords.append(x2);
        coords.append(x3);
        coords.append(x4);
        ...
        */
        x_1 = new QVector3D(3,2,0);
        x_2 = new QVector3D(-2,2,0);
        x_3 = new QVector3D(2,-2,0);
        x_4 = new QVector3D(-3,-2,0);
        x_5 = new QVector3D(0,0,0.2);
...
TEST_F(WellConstraintProjectionsTests, root_alg_test){
    ...
    WellConstraintProjections::real_roots(coeffs,&realroots);
...
TEST_F(WellConstraintProjectionsTests, well_distance_test){
    ...
    double min = 5;
    double min_1 = 5;
    QVector3D* heel = new QVector3D(0,0,0);
    QVector3D* toe = new QVector3D(10,0,0);
    QVector3D* heel_1 = new QVector3D(0,0,0);
    QVector3D* toe_1 = new QVector3D(3,0,0);
...
TEST_F(WellConstraintProjectionsTests, interwell_multiple_wells_test){
    ...
    /*for(int i=0; i<all_wells_moved.length(); i++){
        std::cout << "well " << i << " with coords" << std::endl;
        for (int j=0; j<2; j++){
            std::cout << j << " coord" << std::endl;
            std::cout << all_wells_moved.at(i).at(j) << std::endl;
        }
    }

    std::ofstream myfile;
    myfile.open ("interwell_multiple.txt");
    ...
    for (int i=0; i<n; i++){
        ...
        myfile << "\\addlegendentry{w" << i << "};" << std::endl;
    ...
    myfile << "zmin=-10, zmax=10]" << std::endl;
    for (int i=0; i<n; i++){
        myfile << "\\addplot3 coordinates { (" << all_wells_moved.at(i).at(0)(0) <<"," << all_wells_moved.at(i).at(0)(1) <<"," << all_wells_moved.at(i).at(0)(2) <<
                ") (" << all_wells_moved.at(i).at(1)(0) <<"," << all_wells_moved.at(i).at(1)(1) <<"," << all_wells_moved.at(i).at(1)(2) << ") };" << std::endl;
        myfile << "\\addlegendentry{w" << i << "};" << std::endl;
    }
     myfile << "\\end{axis}" << std::endl;
