...
protected:
    ...
    Grid *grid_;
    //QString file_path_ = "../../../examples/ECLIPSE/ECL_1SPOT_TEST/ECL_5SPOT.EGRID";
...
TEST_F(SingleCellWellIndexTest, WellIndexValue_test) {
    ...
    XYZCoordinate* start_point = new XYZCoordinate(well_start_x,well_start_y,well_start_z);
    XYZCoordinate* end_point= new XYZCoordinate(well_end_x,well_end_y, well_end_z);
    WellIndexCalculator::GeometryClasses::Line* line_ = new WellIndexCalculator::GeometryClasses::Line(start_point,end_point);
    QList<WellIndexCalculator::GeometryClasses::Line*>* lines_ = new QList<WellIndexCalculator::GeometryClasses::Line*>();
    lines_->append(line_);
...
TEST_F(SingleCellWellIndexTest, WellIndexValueWithQVector_test) {
    ...
    double well_end_y = 0.25*corners[4]->y() + 0.25*corners[5]->y() +0.25*corners[6]->y() + 0.25*corners[7]->y();
    double well_end_z = 0.25*corners[4]->z() + 0.25*corners[5]->z() +0.25*corners[6]->z() + 0.25*corners[7]->z();
    QVector3D* start_point = new QVector3D(well_start_x,well_start_y,well_start_z);
    QVector3D* end_point= new QVector3D(well_end_x,well_end_y, well_end_z);
...
TEST_F(SingleCellWellIndexTest, Well_index_grid_test) {
    ...
    QVector3D* start_point = new QVector3D(12,12,1700);
    QVector3D* end_point= new QVector3D(12,12,1724);
    QList<QVector3D*> start_points;
    QList<QVector3D*> end_points;
    start_points.append(start_point);
    end_points.append(end_point);
