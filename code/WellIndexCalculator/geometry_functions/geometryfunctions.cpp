...
{
    Model::Reservoir::Grid::XYZCoordinate *movedPoint = new Model::Reservoir::Grid::XYZCoordinate(input_point->x()+1,input_point->y()+1,input_point->z()+1);
    return movedPoint;
}
...
Model::Reservoir::Grid::XYZCoordinate* GeometryFunctions::line_intersects_plane(GeometryClasses::SquarePlane* plane, GeometryClasses::Line* line)
{
    ...
    Model::Reservoir::Grid::XYZCoordinate* spanning_vec1 = new Model::Reservoir::Grid::XYZCoordinate(plane->c2()->x() - plane->c1()->x(),plane->c2()->y() - plane->c1()->y(),plane->c2()->z() - plane->c1()->z() );
    Model::Reservoir::Grid::XYZCoordinate* spanning_vec2 = new Model::Reservoir::Grid::XYZCoordinate(plane->c3()->x() - plane->c1()->x(),plane->c3()->y() - plane->c1()->y(),plane->c3()->z() - plane->c1()->z() );

    // The cross product of the two spanning vectors is orthogonal to the plane.
    Model::Reservoir::Grid::XYZCoordinate* normal_vector = new Model::Reservoir::Grid::XYZCoordinate(
    ...
    Model::Reservoir::Grid::XYZCoordinate* line_vector = new Model::Reservoir::Grid::XYZCoordinate(line->p1()->x() - line->p0()->x(), line->p1()->y() - line->p0()->y(), line->p1()->z() - line->p0()->z());
    ...
    Model::Reservoir::Grid::XYZCoordinate* line_plane_vector = new Model::Reservoir::Grid::XYZCoordinate(plane->c1()->x() - line->p0()->x(), plane->c1()->y() - line->p0()->y(), plane->c1()->z() - line->p0()->z());
    ...
    Model::Reservoir::Grid::XYZCoordinate* intersecting_point = new Model::Reservoir::Grid::XYZCoordinate(line->p0()->x()+ s*(line_vector->x()), line->p0()->y() + s*(line_vector->y()), line->p0()->z()+ s*(line_vector->z()));
...
Model::Reservoir::Grid::XYZCoordinate* GeometryFunctions::line_intersects_plane_vectorized(GeometryClasses::SquarePlane* plane, GeometryClasses::Line* line){
    ...
    QVector3D* span_vector0 = new QVector3D(plane->c2()->x() - plane->c1()->x(),plane->c2()->y() - plane->c1()->y(),plane->c2()->z() - plane->c1()->z());
    QVector3D* span_vector1 = new QVector3D(plane->c3()->x() - plane->c1()->x(),plane->c3()->y() - plane->c1()->y(),plane->c3()->z() - plane->c1()->z());
    QVector3D* normal_vec = new QVector3D(QVector3D::crossProduct(*span_vector0,*span_vector1));

    QVector3D* line_vec = new QVector3D(line->p1()->x() - line->p0()->x(), line->p1()->y() - line->p0()->y(), line->p1()->z() - line->p0()->z());
    ...
    QVector3D* line_plane_vector = new QVector3D(plane->c1()->x() - line->p0()->x(), plane->c1()->y() - line->p0()->y(), plane->c1()->z() - line->p0()->z());
    ...
    Model::Reservoir::Grid::XYZCoordinate* intersection_point = new Model::Reservoir::Grid::XYZCoordinate(line->p0()->x()+ s*(line_vec->x()), line->p0()->y() + s*(line_vec->y()), line->p0()->z()+ s*(line_vec->z()));
...
QVector3D* GeometryFunctions::line_plane_intersection(QVector3D *p0, QVector3D *p1, QVector3D *normal_vector, QVector3D *point_in_plane){
    ...
    QVector3D *line_vector = new QVector3D(p1->x() - p0->x(), p1->y() - p0->y(), p1->z() - p0->z());
    ...
    QVector3D* w = new QVector3D(p0->x() - point_in_plane->x(), p0->y() - point_in_plane->y(),p0->z() - point_in_plane->z());
    ...
    // Use the found s in parametrizaton to return intersection point.
    QVector3D *intersection_point = new QVector3D(p0->x() + s*(line_vector->x()), p0->y() + s*(line_vector->y()), p0->z() + s*(line_vector->z()));
...
QVector3D* GeometryFunctions::normal_vector(QVector3D *p0, QVector3D *p1, QVector3D *p2)
{
    ...
    QVector3D* normal_vector_ptr = new QVector3D(normal_vector.x(),normal_vector.y(),normal_vector.z());
...
QPair<QList<int>, QList<QVector3D *> > GeometryFunctions::cells_intersected(QVector3D *start_point, QVector3D *end_point, Model::Reservoir::Grid::Grid *grid)
{
    ...
    QVector3D* move_exit_epsilon = new QVector3D(*exit_point*(1-epsilon)+ *end_point*epsilon);

    Model::Reservoir::Grid::Cell* current_cell;
    ...
    while(current_cell->global_index()==first_cell_index){
        ...
        QVector3D* move_exit_epsilon = new QVector3D(*exit_point*(1-epsilon_temp)+ *end_point*epsilon_temp);
        current_cell = GeometryFunctions::get_cell_enveloping_point(grid,move_exit_epsilon);
    }
...
QVector3D* GeometryFunctions::find_exit_point(Model::Reservoir::Grid::Cell *cell, QVector3D *entry_point, QVector3D *end_point, QVector3D *exception_point)
{
    ...
    QVector3D *line = new QVector3D(*end_point-*entry_point);
    ...
    for(int face_number = 0; face_number<6; face_number++){
        ...
        if(!(QVector3D::dotProduct(*cur_normal_vector,*line)==0)) {
            ...
            if (feasible_point && intersect_point->distanceToPoint(*exception_point)>10-10 && QVector3D::dotProduct(*end_point-*entry_point,*end_point-*intersect_point)>=0){
...
GeometryClasses::Line *GeometryFunctions::project_line_to_plane(GeometryClasses::Line *line, GeometryClasses::SquarePlane *plane)
{
    ...
    QVector3D* q0 = new QVector3D(line->p0()->x(),line->p0()->y(),line->p0()->z());
    QVector3D* q1 = new QVector3D(line->p1()->x(),line->p1()->y(),line->p1()->z());
    QVector3D* p  = new QVector3D(plane->c1()->x(),plane->c1()->y(),plane->c1()->z());
    ...
    QVector3D* span_vector0 = new QVector3D(plane->c2()->x() - plane->c1()->x(),plane->c2()->y() - plane->c1()->y(),plane->c2()->z() - plane->c1()->z());
    QVector3D* span_vector1 = new QVector3D(plane->c3()->x() - plane->c1()->x(),plane->c3()->y() - plane->c1()->y(),plane->c3()->z() - plane->c1()->z());
    QVector3D* orth_vec = new QVector3D(QVector3D::crossProduct(*span_vector0,*span_vector1));
    ...
    Model::Reservoir::Grid::XYZCoordinate* returnpoint0 = new Model::Reservoir::Grid::XYZCoordinate(proj_p0.x(),proj_p0.y(),proj_p0.z());
    Model::Reservoir::Grid::XYZCoordinate* returnpoint1 = new Model::Reservoir::Grid::XYZCoordinate(proj_p1.x(),proj_p1.y(),proj_p1.z());
    GeometryClasses::Line* returnline = new GeometryClasses::Line(returnpoint0,returnpoint1);
...
QVector3D *GeometryFunctions::project_point_to_plane(QVector3D *point, QVector3D *normal_vector, QVector3D *plane_point)
{
    ...
    QVector3D *ptr_proj_point = new QVector3D(proj_point);
...
QVector3D* GeometryFunctions::project_v1_on_v2(QVector3D* v1, QVector3D* v2)
{
    ...
    QVector3D* ptr_proj_v = new QVector3D(proj_v);
    return ptr_proj_v;
}

QVector3D* GeometryFunctions::XYZ_to_qvec(Model::Reservoir::Grid::XYZCoordinate* coor)
{
    QVector3D* vec = new QVector3D(coor->x(),coor->y(),coor->z());
    return vec;
}
...
Model::Reservoir::Grid::XYZCoordinate* GeometryFunctions::qvec_to_XYZ(QVector3D* vec)
{
    ...
    Model::Reservoir::Grid::XYZCoordinate* xyz = new Model::Reservoir::Grid::XYZCoordinate(vector.x(),vector.y(),vector.z());
    return xyz;
}
...
{
    QVector3D* vector = new QVector3D((p2->x() ) -( p1->x()), (p2->y() ) - (p1->y()), (p2->z()) - (p1->z()));
    return vector;
}
...
double GeometryFunctions::well_index_cell_qvector(Model::Reservoir::Grid::Cell *cell, QList<QVector3D*> start_points, QList<QVector3D*> end_points, double wellbore_radius)
{
    ...
    for (int ii = 0; ii < start_points.length(); ++ii) {
        // Current segment ii
        // Compute vector from segment
        QVector3D* current_vec = new QVector3D(*end_points.at(ii)-*start_points.at(ii));
